...
export function createMsteamsStreamingCall(params) {
    ...
    async function runTurn(question, opts) {
        ...
        if (opts?.gated && !addressed(question)) {
            ...
            log?.debug?.(`[msteams-bridge] streaming: caller turn not addressed to the bot — not answering (${session.callId})`);
            return;
        }
        processing = true;
        try {
    ...
    const sttSession = deps.createTranscriptionSession?.({
        ...
        onError: (e) => log?.warn(`[msteams-bridge] streaming STT session error for ${session.callId}: ${e.message}`),
    });
    if (sttSession) {
        void sttSession
            .connect()
            .catch((e) => log?.warn(`[msteams-bridge] streaming STT connect failed for ${session.callId}: ${String(e)}`));
    }
    return {
        pushAudio: (pcm16k) => {
            if (closed)
